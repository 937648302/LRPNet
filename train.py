...
# read dataset
print('trainset load.....')
trainset = dataset.Cherry(root='./cherry-dataset', is_train=True, data_len=None)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=BATCH_SIZE,
                                          shuffle=True, num_workers=8, drop_last=False)
print('testset load.....')
testset = dataset.Cherry(root='./cherry-dataset', is_train=False, data_len=None)
testloader = torch.utils.data.DataLoader(testset, batch_size=BATCH_SIZE,
                                         shuffle=False, num_workers=8, drop_last=False)
print('validationset load.....')
validationset = dataset.Cherry(root='./cherry-dataset', is_train=False, is_validation=True, data_len=None)
    ...
    # evaluate on validation set
        ...
        for i, data in enumerate(validationloader):
            with torch.no_grad():
                ...
                validation_correct += torch.sum(concat_predict.data == label.data)
                validation_loss += concat_loss.item() * batch_size
                progress_bar(i, len(validationloader), 'eval validation set')

        validation_acc = float(validation_correct) / total
